...
bool enhanced_recover_with_verification(const usb_device_info_t *device,
    ...
    /* SECURITY: Strict signature verification using crypto module */
    int verify_result = crypto_verify(firmware_buffer, firmware_size,
                                     signature, sig_size);
    
    if (verify_result != CRYPTO_SUCCESS) {
        free(firmware_buffer);
        if (verify_result == CRYPTO_ERROR_SIGNATURE_INVALID) {
            strncpy(verification->error_message, "Signature verification failed - invalid signature",
                    sizeof(verification->error_message) - 1);
        } else {
            strncpy(verification->error_message, "Signature verification failed - cryptographic error",
                    sizeof(verification->error_message) - 1);
        }
...
bool enhanced_verify_recovery(const uint8_t *firmware, size_t size,
    ...
    /* SECURITY: Calculate hash using crypto module */
    int hash_result = crypto_sha256(firmware, size, verification->firmware_hash);
    if (hash_result != CRYPTO_SUCCESS) {
        strncpy(verification->error_message, "Hash calculation failed",
                sizeof(verification->error_message) - 1);
        return false;
    }
    ...
    /* SECURITY: Signature verification must be performed with stored public key
     * This is a placeholder - in production, this should verify against stored key */
    /* For now, we require that signature verification was done before calling this */
    verification->signature_valid = true;  // Assumed valid if we got here
